...
from logica_paciente import crear_paciente, listar_pacientes, consultar_pordatos, editar_paciente, eliminar_paciente, buscar_por_id_editar, validar_duplicado
...
"""Request es un objeto de Flask que contiene toda la información de la petición que llegó.
    ...
    request.form        → los datos del formulario y esto es POST y e mas seguro
    request.form.get("nombre")  → el valor del campo "nombre"
    request.url         → la URL completa
    request.args        → parámetros de la URL (?buscar=juan) esto es GET
 """
...
def listar_pacientes_route ():
    ...
    if valor_buscar is None or valor_buscar == "":
        ...
        lista_consultada = consultar_pordatos(valor_buscar)
        return render_template("pacientes.html", usuario=current_user, pacientes=lista_consultada)

@blueprint_pacientes.route("/pacientes/crear-paciente",methods=["POST"]) #Get es para traer la pagina y POST es para enviar los datos del formulario
@login_required
#request es un objeto de Flask que contiene toda la información de la petición que llegó
...
@blueprint_pacientes.route("/pacientes/validar-documento",methods=["POST"]) #Get es para traer la pagina y POST es para enviar los datos del formulario
@login_required
#request es un objeto de Flask que contiene toda la información de la petición que llegó
def validar_duplicado_route():
    if request.method == "POST":
        if validar_duplicado(request.form.get("documento")) == True:
            ...
            flash("El documento no está registrado")
            return redirect(url_for("bluep_pacientes.listar_pacientes_route")) #1 va en blueprint_pacientes porque es el nombre del blueprint y 2 va en pacientes porque es el nombre de la funcion que tenemos arriba
